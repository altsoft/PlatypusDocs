...
required. Such keys may come from database structure definitions.
------------------------------------
...
|`onDelete` |`DeleteEvent` |Is called after removing an object from the
entitiy's data array.

|`onInsert` |`InsertEvent` |Is called after inserting a new object
into the entitiy's data array.

|`onScroll` |`ChangeEvent` |Is called after changing the cursor property
in the entity's data array

|`onChange` |`ChangeEvent` |Is called after changing the element of
the entity's data array
|=======================================================================
...
|==============================
|Property |Description
|`source` |The data array, the deletion takes place in.
|`items` |The deleted objects.
|==============================
...
|================================
|Property |Description
|`source` |The data array, the insertion takes place in.
|`items` |The inserted objects.
|================================
...
Use `update()` entity method to execute the SQL code, which contains
the `Insert`, `Update`, `Delete` or Ddl commands:

[source,Javascript]
----------------------------------------------------------------------------

// model.deleteAt entity is Dml based entity with the following Sql:
// "Delete From Document d Where d.docType = :docType"
model.deleteAt.update({docType: 50}, function(){
    // ...
}, function(anError){
    // ...
});
----------------------------------------------------------------------------
The `update()` entity method takes first argument as parameters object for dml query's parameters.

NOTE: This method goes in its own transaction.

Use `enqueueUpdate()` entity method to prepare Sql code execution, which contains
the `Insert`, `Update`, `Delete` or Ddl commands:
...
// model.deleteAt entity is Dml based entity with the following Sql:
// "Delete From Document d Where d.docType = :docType"
model.deleteAt.enqueueUpdate({docType: 50});
model.save(function(){
    // ...
}, function(anError){
    // ...
...
----------------------------------------------------------------------------
The `enqueueUpdate(params)` entity method takes first argument as parameters object for dml query's parameters.
The `params` argument is optional. So if it is absent, current values of entity's parameters will be taken instead.
This method prepares a record about executed command in client's change log wich will be applied while next commit operation.
Note, that every `model.save(...)` call commits accumulated changes and consumes client's change log.

Legacy `executeUpdate()` entity method executes a Sql code, which contains
the `Insert`, `Update`, `Delete` or Ddl commands:

[source,Javascript]
----------------------------------------------------------------------------

var e = model.createEntity("DELETE * FROM Document d WHERE d.DOC_TYPE = :docType");
e.params.docType = 50;
e.executeUpdate(function(){
    // ...
}, function(){
    // ...
});
...
The `executeUpdate()` entity method takes entity's parameters as dml query's parameters.

NOTE: This method goes in its own transaction.

NOTE: createEntity method may be used only in two-tier environment. E.g. in Java EE
server, J2SE client or in Platypus TSA server.

NOTE: This method is deprecated, and so, use `update` method instead.
...
There are three levels of parallel execution in Platypus.js server environment:

* Resident modules level.
* Session modules level.
* Stateless modules level.

Each level of parallel execution considers its own script spaces (script context and global object) magaging policy.
...
LPC allows one module from one level to make calls to another module from another level of parallel execution and
