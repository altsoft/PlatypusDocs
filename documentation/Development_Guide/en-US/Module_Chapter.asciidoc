...
Modules can be either global modules and https://en.wikipedia.org/wiki/Asynchronous_module_definition[AMD] modules.

Every AMD module contains strictly one call to a global function define.
Define function should be used as follows:
 ...
 *
 */
define(['logger', 'other-super-module'], function(Logger, OtherModule){
...
The @module annotation allows to assign a short name to the defined module.
If this annotation is absent, module name will be formed as relative path from
'app' directory in application project and without .js filename extension.
Such modules names have no the leading slash. So if you have a module without 
@module annotation in file 'app/folder/module.js', the module name will be 'folder/module'

The define function of the module is called only once per script global object (script space).
So it is easy to implement such pattern as singleton and JavaScript class pattern.
...
Dependencies of a module may be referenced if first argument of define function as relative module names.
...
Relative module names may formed using filesystem reserved directories names ('../', '../../', './').

Every global module contains a constructor function. This function is used for
...
* Create a proxy/stub for the server module, to use RPC on client or Local procudure call (LPC) on an apllication server:
+
[source,Javascript]
---------------------------------------- 
...
Dependencies resolution for AMD modules is base on AMD specification.
...
or an array of these elements, callback is a function,
...
NOTE: If the relative module name is equals to @module annotation value, only the annotated module
will be taken into accout while dependency resolution process.
...
[source,Javascript]
-------------------------------------- 
require("./sublibs/sublib-a.js");
// SubLibObjectA is defined in sublib-a.js
var obja = new SubLibObjectA();
require("./sublibs/sublib-b.js", function(SubLibObjectB){
...
(Either in define call JsDoc section or constructor's JsDoc section).
...
The object of information about the user contains the `name`
...
NOTE: Platypus.js supports asynchronous IO model and so maxThreads parameter should be configured
