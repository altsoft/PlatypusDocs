...
'app' directory in application project and without .js filename extension without leading slash.
So if you have a module without  @module annotation in file 'app/folder/module.js', the module name will be 'folder/module'.
...
Relative module names may be formed using filesystem reserved directories names ('../', '../../', './').
...
Typically, `define`'s callback is returns module definition as JavaScript "class" constructor. 
Tis constructor will be used by depending code to create module's "class" instances.
On the other hand, `define`'s callback is free to return plaim JavaScript object and depending code will
recieve exactly this object regardless of how many times it is required.
