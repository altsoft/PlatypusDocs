...
To run the plafrom's application in a J2EE servlet container or on the
...
web-application `WEB-INF/lib` directory from the `bin` sub-directory of
...
for example `jdbc/main`. Configure the connection pool and JNDI
...
* Set the initialization parameter and specify `url` as its name and
...
...
--------------------------------------------------------
* Set the initialization parameter and specify `default-datasource` as
...
[[apache-tomcat-7-configuration-j2ee]]
